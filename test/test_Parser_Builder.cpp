...
const std::regex decodeReg("\\^A");
const std::regex encodeReg("\\\x01");
...
TEST(RoundTripTest, ParserExtractsBeginString) {
  ...
  flm::fix::FixMessage::parse(decodeFix(SAMPLE_NEW_ORDER_SINGLE), msg, noGroupDefs());
...
TEST(RoundTripTest, CancelReplaceExtractsQty) {
  flm::fix::FixMessage msg;
  flm::fix::FixMessage::parse(decodeFix(SAMPLE_CANCEL_REPLACE), msg, noGroupDefs());
  auto val = msg.getInt(flm::fix::FieldTag::OrderQty);
  ASSERT_TRUE(val.has_value());
  EXPECT_EQ(*val, 2000);
}
...
TEST(RoundTripTest, LimitOrderExtractsLongClOrdID) {
  flm::fix::FixMessage msg;
  flm::fix::FixMessage::parse(decodeFix(SAMPLE_LIMIT_ORDER), msg, noGroupDefs());
  auto val = msg.getString(flm::fix::FieldTag::ClOrdID);
  ASSERT_TRUE(val.has_value());
...
TEST(RoundTripTest, ParseBuildParsePreservesClOrdID) {
  ...
  auto clOrdID = msg1.getString(flm::fix::FieldTag::ClOrdID);
  ASSERT_TRUE(clOrdID.has_value());

  flm::fix::Builder b;
  b.addField(flm::fix::FieldTag::ClOrdID, *clOrdID);
  b.addField(flm::fix::FieldTag::OrderQty, int64_t(100));
  const std::string built{b.build()};

  flm::fix::FixMessage msg2;
  flm::fix::FixMessage::parse(built, msg2, defs);
  auto clOrdID2 = msg2.getString(flm::fix::FieldTag::ClOrdID);
  ASSERT_TRUE(clOrdID2.has_value());
  EXPECT_EQ(*clOrdID, *clOrdID2);
}
...
TEST(RoundTripTest, ParseBuildParsePreservesMultipleFields) {
  ...
  flm::fix::FixMessage msg2;
  flm::fix::FixMessage::parse(built, msg2, defs);
  EXPECT_EQ(msg2.getString(flm::fix::FieldTag::ClOrdID).value_or(""), *clOrdID);
...
TEST(RoundTripTest, LogonFieldsPreservedAfterRebuild) {
  ...
  auto msgType = msg.getString(flm::fix::FieldTag::MsgType);
  ASSERT_TRUE(sender.has_value());
  ASSERT_TRUE(target.has_value());
  ASSERT_TRUE(msgType.has_value());

  flm::fix::Builder b;
  b.addField(flm::fix::FieldTag::MsgType, *msgType);
...
TEST(RoundTripTest, LimitOrderCriticalFieldsSurviveRebuild) {
  ...
  auto clOrdID = msg.getString(flm::fix::FieldTag::ClOrdID);
