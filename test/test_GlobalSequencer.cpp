...
TEST_F(GlobalSequencerTest, ConstructorInitializesNonZeroEpoch) {
  ...
  EXPECT_GT(seq.nextTimestampNs(), 0ULL);
}

TEST_F(GlobalSequencerTest, GeneratesMonotonicTimestamps) {
  uint64_t t1 = sequencer_.nextTimestampNs();
  uint64_t t2 = sequencer_.nextTimestampNs();
  uint64_t t3 = sequencer_.nextTimestampNs();

  // Consecutive calls increment by exactly 1 — delta is what matters,
  // not absolute value (epoch_ns_ is an arbitrary nanosecond timestamp)
  EXPECT_EQ(t2 - t1, 1ULL);
  EXPECT_EQ(t3 - t2, 1ULL);
}

TEST_F(GlobalSequencerTest, SequentialCallsIncrementByOne) {
  uint64_t prev = sequencer_.nextTimestampNs();
  for (int i = 0; i < 1000; ++i) {
    uint64_t curr = sequencer_.nextTimestampNs();
  ...
  for (int t = 0; t < NUM_THREADS; ++t) {
    threads.emplace_back([&, t]() {
      for (int i = 0; i < CALLS_PER_THREAD; ++i)
        per_thread[t].push_back(sequencer_.nextTimestampNs());
    });
  }
...
TEST_F(GlobalSequencerTest, PerformanceSingleThread) {
  ...
  for (int i = 0; i < CALLS; ++i) sequencer_.nextTimestampNs();
...
TEST_F(GlobalSequencerTest, MultipleInstancesIndependent) {
  ...
  uint64_t t1_a = seq1.nextTimestampNs();
  uint64_t t2_a = seq2.nextTimestampNs();
  uint64_t t1_b = seq1.nextTimestampNs();
  uint64_t t2_b = seq2.nextTimestampNs();

  // Each instance has its own seq_counter starting at 0 and its own epoch_ns_.
...
TEST_F(GlobalSequencerTest, SequentialAfterThreadStress) {
  ...
  for (auto& t : threads) {
    t = std::thread([this]() {
      for (int i = 0; i < 10000; ++i) sequencer_.nextTimestampNs();
    });
  }
  for (auto& t : threads) t.join();

  // After concurrent stress, sequential increments must still be exactly 1
  uint64_t a = sequencer_.nextTimestampNs();
  uint64_t b = sequencer_.nextTimestampNs();
  EXPECT_EQ(b - a, 1ULL);
}