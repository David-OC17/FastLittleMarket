...
static std::string readLog(const std::string& path) {
...
TEST_F(ExchangeLoggerTest, LogInfo) {
  ...
  EXPECT_TRUE(logContains(log, "LOG_INFO")) << "Log level tag missing";
}
...
TEST_F(ExchangeLoggerTest, LogWarning) {
  ...
  EXPECT_TRUE(logContains(log, "LOG_WARNING")) << "Log level tag missing";
}
...
TEST_F(ExchangeLoggerTest, LogError) {
  ...
  EXPECT_TRUE(logContains(log, "LOG_ERROR")) << "Log level tag missing";
}
...
TEST_F(ExchangeLoggerTest, MultipleThreadsWriteToSameLog) {
  ...
  for (int t = 0; t < NUM_THREADS; ++t) {
    EXPECT_TRUE(logContains(log, "thread_" + std::to_string(t) + "_entry_0"))
        << "Missing first entry from thread " << t;
    EXPECT_TRUE(logContains(log, "thread_" + std::to_string(t) + "_entry_" +
...
TEST_F(ExchangeLoggerTest, MacroNewOrderSingle) {
  auto order = makeOrder(1, 1000000, 100, flm::BUY_SIDE);
  ...
  EXPECT_TRUE(logContains(log, "price_q4=1000000"));
  EXPECT_TRUE(logContains(log, "vol=100"));
  EXPECT_TRUE(logContains(log, "client=JPMORG"));
}

TEST_F(ExchangeLoggerTest, MacroCancelOrderRequest) {
  auto order = makeOrder(2, 1000000, 50, flm::SELL_SIDE);
  ...
  EXPECT_TRUE(logContains(log, "vol=50"));
}

TEST_F(ExchangeLoggerTest, MacroMatch) {
  auto passive = makeOrder(3, 1000000, 100, flm::BUY_SIDE);
  auto aggressive = makeOrder(4, 990000, 40, flm::SELL_SIDE);
  ...
  EXPECT_TRUE(logContains(log, "fill_vol=40"));
  EXPECT_TRUE(logContains(log, "fill_price_q4=1000000"));
}

TEST_F(ExchangeLoggerTest, MacroOrderAccepted) {
  auto order = makeOrder(5, 1050000, 200, flm::BUY_SIDE);
...
TEST_F(ExchangeLoggerTest, MacroOrderRejected) {
  auto order = makeOrder(6, 0, 0, flm::BUY_SIDE);  // invalid order
...
TEST_F(ExchangeLoggerTest, MacroOrderReplaced) {
  auto old_order = makeOrder(7, 1000000, 100, flm::BUY_SIDE);
  auto new_order = makeOrder(8, 1010000, 80, flm::BUY_SIDE);
  ...
  EXPECT_TRUE(logContains(log, "old_price_q4=1000000"));
  EXPECT_TRUE(logContains(log, "new_price_q4=1010000"));
  EXPECT_TRUE(logContains(log, "old_vol=100"));
  EXPECT_TRUE(logContains(log, "new_vol=80"));
}

TEST_F(ExchangeLoggerTest, MacroOrderCanceled) {
  auto order = makeOrder(9, 1000000, 75, flm::SELL_SIDE);
  ...
  EXPECT_TRUE(logContains(log, "SELL"));
  EXPECT_TRUE(logContains(log, "vol=75"));
}

TEST_F(ExchangeLoggerTest, MacroOrderExecuted) {
  auto order = makeOrder(10, 1000000, 100, flm::BUY_SIDE);
  ...
  EXPECT_TRUE(logContains(log, "leaves_vol=40"));
  EXPECT_TRUE(logContains(log, "fill_price_q4=1000000"));
}