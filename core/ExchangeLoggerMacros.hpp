...
#define FLM_SIDE_STR(order) ((order).side == fast_little_market::BUY_SIDE ? "BUY" : "SELL")
#define FLM_TYPE_STR(order) ((order).type == fast_little_market::MARKET_TYPE ? "MKT" : "LMT")
#define FLM_TIF_STR(order) \
  ((order).tim == fast_little_market::DAY_TIF ? "DAY" : (order).tim == fast_little_market::GTC_TIF ? "GTC" : "IOC")
